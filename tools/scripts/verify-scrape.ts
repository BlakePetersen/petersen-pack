...
async function verifyScrapedData() {
  ...
  galleries.forEach((gallery) => {
    ...
    console.log(`  ${gallery.title.padEnd(30)} ${imageCount.toString().padStart(5)} images`);
  });

  console.log('-'.repeat(60));
  console.log(`  ${'TOTAL'.padEnd(30)} ${totalImages.toString().padStart(5)} images`);
  console.log('\n' + '='.repeat(60));
  ...
  if (fs.existsSync(scrapedDir)) {
    const files = fs.readdirSync(scrapedDir).filter((f: string) => f.endsWith('.webp'));
    console.log(`  ✓ Found ${files.length} WebP files in /public/uploads/scraped/`);

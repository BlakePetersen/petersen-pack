// ABOUTME: Gallery unpublish POST — composed wrapper chain per SEC-07
...
export async function POST(
  ...
  const handler = withRequestContext(
    withRateLimit(
      ...
      withCsrf(
        withAdminAuth(
          withAudit('gallery.unpublish', async (_req, ctx) => {
            try {
              ...
              return await prisma.$transaction(async (tx) => {
                ...
                await tx.auditLog.create({
                  data: {
                    ...
                    afterJson: { status: gallery.status },
                  },
                })
                return NextResponse.json(gallery)
              })
            } catch (error) {
              logger.error({ err: error }, 'Failed to unpublish gallery')
              return NextResponse.json(

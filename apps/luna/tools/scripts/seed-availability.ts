...
async function seedAvailability() {
  ...
  for (const slot of slots) {
    try {
      ...
      if (existing) {
        console.log(
          `  ⊘ Skipped: ${formatDate(slot.date)} ${slot.startTime}-${slot.endTime} (already exists)`
        )
        skipped++
      } else {
        ...
        console.log(
          `  ✓ Created: ${formatDate(slot.date)} ${slot.startTime}-${slot.endTime}`
        )
        created++
      }
    } catch (error) {
      console.error(
        `  ✗ Failed to create slot for ${formatDate(slot.date)}:`,
  ...
  allSlots.slice(0, 5).forEach((slot) => {
    ...
    const dayOfWeek = slot.date.toLocaleDateString('en-US', {
      weekday: 'short',
    })
    const available = slot.isAvailable ? '✓' : '✗'
    console.log(
      `  ${available} ${dayOfWeek} ${dateStr} ${slot.startTime}-${slot.endTime}`
    )
    if (slot.notes) {

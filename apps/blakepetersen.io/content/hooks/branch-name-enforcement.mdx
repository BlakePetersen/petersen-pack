...
This hook lives at `.husky/pre-push` (or installs alongside an existing pre-push hook — see the merge note below). It exits zero for protected branches and convention-matching branches, exits non-zero with a helpful message for everything else.
...
<a href="/install/hooks/branch-name-enforcement" target="_blank" rel="noopener">View full .husky/pre-push →</a>
...
## Coexisting with Other Pre-push Hooks

If `.husky/pre-push` already exists — for instance, the pre-push-validation hook from this same plan — the branch check needs to merge cleanly. The recommended layout is to put the branch check first (it's the fastest check, and a failure here means the slower checks would have wasted time):

```sh title=".husky/pre-push (combined)"
# 1. Branch-name check (fail fast)
case "$branch" in
  main|master|develop|staging|production|feat/*|fix/*|chore/*|docs/*|refactor/*|test/*|perf/*|build/*|ci/*|revert/*|gsd/*)
    ;;
  *)
...
The combined form folds the branch check into one `case` statement with the allow-patterns in the matching branch and the rejection in the default. That's the same shape as the standalone artifact, just inverted — the standalone exits 0 from matched branches and falls through to the rejection; the combined exits 0 in the same way and lets execution continue to the next check on success.

...
A neighboring choice is a `case "$changed" in ...pnpm-lock.yaml...)` glob match. The glob is shorter and equivalent in behavior, but the surrounding asterisks are easy to lose to an over-eager Markdown formatter that sees them as italics. The `grep -q` form is one line longer and immune to that class of accident, which is the right trade for content that lives in `.md` siblings.

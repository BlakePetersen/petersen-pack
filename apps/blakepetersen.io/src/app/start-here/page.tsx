...
export const metadata: Metadata = {
  title: 'Start Here',
  description:
    'A guided introduction to the DX practices documented on this site.'
}
...
const steps: Step[] = [
  {
    ...
    why: 'Consistent code style is the foundation everything else builds on.'
  },
  {
    collection: 'hooks',
    slug: 'hooks/pre-commit/lint-staged-setup',
    why: 'Automated quality gates on every commit.'
  },
  {
    collection: 'guides',
    slug: 'guides/monorepo-setup',
    why: 'Structure your project for scale from day one.'
  },
  {
    collection: 'skills',
    slug: 'skills/claude-code/writing-custom-skills',
    why: 'Teach your AI assistant project-specific patterns.'
  }
]
    ...
    <div className="mx-auto max-w-[72ch] px-4 py-12">
      ...
      {resolved.length > 0 ? (
        <ol id="steps" className="mt-12 space-y-6">
          {resolved.map(step => (
            <li key={step.href} className="bg-card p-6 border border-border">

...
const ARTIFACT_PATH = path.join(FIXTURE, 'content', 'configs', 'sample-config.artifact.md')
  ...
  it(
    ...
    () => {
      ...
      if (withFmEdit === baseline) {
        throw new Error('Test setup: could not locate description: in fixture frontmatter')
      }
      ...
      if (withBodyEdit === withFmEdit) {
        throw new Error('Test setup: could not append body content past fixture frontmatter close')
      }

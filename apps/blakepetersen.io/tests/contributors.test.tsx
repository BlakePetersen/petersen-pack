...
jest.mock('@/components/contributor-card', () => ({
  ContributorCard: ({ contributor }: { contributor: { login: string } }) =>
    contributor.login
}))
...
function findAll(
  node: JsxNode,
  predicate: (n: React.ReactElement) => boolean
): React.ReactElement[] {
  ...
  const children = (node as React.ReactElement).props?.children as
    | JsxNode
    | undefined
  if (children != null) {
...
describe('contributors page', () => {
  ...
  it('orders contributors by commit count descending', async () => {
    ...
    const tree = await ContributorsPage()
    const cards = findAll(tree, n => n?.props?.contributor)
    expect(cards.map(c => c.props.contributor.login)).toEqual([
      'high',
      'mid',
      'low'
    ])

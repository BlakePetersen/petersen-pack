// ABOUTME: Site footer component
// ABOUTME: Contains contact info, navigation links, and social media

import Link from 'next/link'
import { LogoBranding } from '@/components/commons/LogoBranding'

function Footer() {
  return (
    <footer className="border-t bg-gray-900 text-gray-300">
      <div className="container mx-auto p-gutter py-section-sm">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-4">
          {/* Brand */}
          <div>
            <LogoBranding
              size={48}
              linkable={false}
              primaryTextClassName="text-gray-400"
              secondaryTextClassName="text-white"
              className="mb-4"
            />
            <p className="text-sm">
              Professional photography services in the East Bay, San Francisco,
              and Contra Costa County
            </p>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="mb-4 font-semibold text-white">Quick Links</h4>
            <div className="flex flex-col gap-2">
              <Link href="/portfolio" className="hover:text-white">
                Portfolio
              </Link>
              <Link href="/about" className="hover:text-white">
                About
              </Link>
              <Link href="/services" className="hover:text-white">
                Services
              </Link>
              <Link href="/blog" className="hover:text-white">
                Blog
              </Link>
              <Link href="/contact" className="hover:text-white">
                Contact
              </Link>
              <Link href="/client-portal" className="hover:text-white">
                Client Portal
              </Link>
              <Link href="/admin" className="hover:text-white">
                Admin Portal
              </Link>
            </div>
          </div>

          {/* Services */}
          <div>
            <h4 className="mb-4 font-semibold text-white">Services</h4>
            <div className="flex flex-col gap-2 text-sm">
              <p>Family Portraits</p>
              <p>Headshots</p>
              <p>Commercial</p>
              <p>Creative Sessions</p>
              <p>Events</p>
            </div>
          </div>

          {/* Contact */}
          <div>
            <h4 className="mb-4 font-semibold text-white">Get in Touch</h4>
            <div className="flex flex-col gap-2 text-sm">
              <p>East Bay, CA</p>
              <a href="tel:925-289-9573" className="hover:text-white">
                925-289-9573
              </a>
              <Link href="/contact" className="hover:text-white">
                Book a Session →
              </Link>
            </div>
            <div className="mt-4 flex gap-3">
              <a
                href="https://www.instagram.com/ashleypetersenphoto"
                target="_blank"
                rel="noopener noreferrer"
                className="text-gray-400 hover:text-white"
                aria-label="Instagram"
              >
                <svg
                  className="h-5 w-5"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
              <a
                href="https://www.facebook.com/ashleypetersenphoto"
                target="_blank"
                rel="noopener noreferrer"
                className="text-gray-400 hover:text-white"
                aria-label="Facebook"
              >
                <svg
                  className="h-5 w-5"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
            </div>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-gray-800 pt-8 text-sm md:flex-row">
          <p>
            © {new Date().getFullYear()} Ashley Petersen Photography. All
            rights reserved.
          </p>
          <div className="flex items-center gap-6">
            <Link
              href="/client-portal"
              className="text-gray-400 transition-colors hover:text-white"
            >
              Client Portal
            </Link>
          </div>
        </div>
      </div>
    </footer>
  )
}
